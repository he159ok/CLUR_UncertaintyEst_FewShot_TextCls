...
from train.metric_loss import multiclass_metric_loss, metric_loss
...
def cal_dif_list_tensors(list_tensors):
    list_len = len(list_tensors)
    res = np.ones(list_len-1)
    for i in range(1, list_len):
        mid = list_tensors[i] - list_tensors[0]
        res[i-1] = torch.sum(torch.norm(mid, p=1)).item() / torch.sum(torch.norm(list_tensors[0] + 0.000000000001, p=1)).item()
    return res

def get_visulize_statistics(train_data, val_data, model, args):
    '''
        Train the model
        Use val_data to do early stopping
    '''
    ...
    for ep in range(1):
        ...
        XS1_dif_array, alphaS1_dif_array, XQ1_dif_array, alphaQ1_dif_array, Q_pred1_dif_array, Q_logit_dif_array = None, None, None, None, None, None
        ...
        for task in sampled_tasks:
            ...
            XS1_list, alphaS1_list, XQ1_list, alphaQ1_list, Q_pred1_list, Q_logit_list =\
                get_visulize_statistics_one(task, model, opt, args, grad)



            if XS1_dif_array is None:
                XS1_dif_array = cal_dif_list_tensors(XS1_list)
                alphaS1_dif_array = cal_dif_list_tensors(alphaS1_list)
                XQ1_dif_array = cal_dif_list_tensors(XQ1_list)
                alphaQ1_dif_array = cal_dif_list_tensors(alphaQ1_list)
                Q_pred1_dif_array = cal_dif_list_tensors(Q_pred1_list)
                Q_logit_dif_array = cal_dif_list_tensors(Q_logit_list)
            else:
                XS1_dif_array += cal_dif_list_tensors(XS1_list)
                alphaS1_dif_array += cal_dif_list_tensors(alphaS1_list)
                XQ1_dif_array += cal_dif_list_tensors(XQ1_list)
        ...
        XS1_dif_array_mean = XS1_dif_array/jsq
        alphaS1_dif_array_mean = alphaS1_dif_array/jsq
        XQ1_dif_array_mean = XQ1_dif_array/jsq
        alphaQ1_dif_array_mean = alphaQ1_dif_array/jsq
        Q_pred1_dif_array_mean = Q_pred1_dif_array/jsq
        Q_logit_dif_array_mean = Q_logit_dif_array/jsq

        res = {}
        res['XS1_dif_array_mean'] = XS1_dif_array_mean
        res['alphaS1_dif_array_mean'] = alphaS1_dif_array_mean
        res['XQ1_dif_array_mean'] = XQ1_dif_array_mean
        res['alphaQ1_dif_array_mean'] = alphaQ1_dif_array_mean
        res['Q_pred1_dif_array_mean'] = Q_pred1_dif_array_mean
        res['Q_logit_dif_array_mean'] = Q_logit_dif_array_mean

        save_np_name = 'np_difference_mask_evidiences.npy'
        np.save(save_np_name, res)
        print(f'diff res has been saved to {save_np_name}')
        print(XS1_dif_array_mean)
        print(alphaS1_dif_array_mean)
        print(XQ1_dif_array_mean)
        print(alphaQ1_dif_array_mean)
        print(Q_pred1_dif_array_mean)
        print(Q_logit_dif_array_mean)
        return




def get_visulize_statistics_one(task, model, opt, args, grad):
    '''
        Train the model on one sampled task.
    '''
    model['ebd'].train()
    ...
    for mask_ratio in [0.9999999, 0.9, 0.8, 0.7, 0.6, 0.5, 0.4, 0.3, 0.2, 0.1, 0]:
        XS1, TS_onehot1, alphaS1 = model['ebd'].visualize(support, lowb=mask_ratio)
        YS = support['label']
        XS1_list.append(XS1)
        TS_onehot1_list.append(TS_onehot1)
        alphaS1_list.append(alphaS1)
        YS_list.append(YS)


        XQ1, TQ_onehot1, alphaQ1 = model['ebd'].visualize(query, lowb=mask_ratio)
        YQ = query['label']
        XQ1_list.append(XQ1)
        TQ_onehot1_list.append(TQ_onehot1)
        alphaQ1_list.append(alphaQ1)
        YQ_list.append(YQ)


        if args.modelmode == "acl":
            C_YS, C_YQ, Q_pred1, Q_logit1 = model['clf'].visualize(XS1, YS, XQ1, YQ, mixup_y=TS_onehot1,
                                                                        mixup_y2=None)
            C_YS_list.append(C_YS)
            C_YQ_list.append(C_YQ)
            Q_pred1_list.append(Q_pred1)
            Q_logit_list.append(Q_logit1)


    return XS1_list, alphaS1_list, XQ1_list, alphaQ1_list, Q_pred1_list, Q_logit_list
